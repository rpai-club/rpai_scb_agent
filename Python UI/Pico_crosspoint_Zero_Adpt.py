...
# For pi pico Cross Point Zero Adpt
# Four analog + 2 AWG channel scope
...
ADC_Cal = 3.29
AWGPeakToPeak = 8.0 # 4.095 # Iternal reference voltage for MCP4822
#
...
MatrixStatus.set(0)
AWG_Amp_Mode.set(0)
DevID = "Pi Pico Zero Cross Point Adpt"
SerComPort = 'Auto'
TimeSpan = 0.01
InterpRate = 4
...
def BB_test():
    ...
    """
    global ser, CompString, JumperString, OnOffString, NumConn, TRACESread
...
def ConnectDevice():
    ...
    if DevID == "No Device" or DevID == "Pi Pico Zero Cross Point Adpt":
        #
        if SerComPort == 'Auto':
        ...
        # Check if hardware ID string matched target?
                        ...
                        ID = ID.replace("'","")
                        print("ID string ", ID)
                        if ID == "Pi Pico Zero Cross Point Adpt" :
                            break
                    except:
...
def AWGASendWave(AWG3):
    ...
    AWGAAmplvalue = AWGAAmplvalue + 4.0
    AWGAOffsetvalue = AWGAOffsetvalue + 4.0
...
def AWGBSendWave(AWG3):
    ...
    AWGBAmplvalue = AWGBAmplvalue + 4.0
    AWGBOffsetvalue = AWGBOffsetvalue + 4.0
